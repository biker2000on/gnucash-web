...
export interface ScheduleFMappingAccount {
    guid: string;
    name: string;
    fullname: string;
    accountType: string;
    /** Keyword-heuristic line, or null when nothing matched (→ line 32). */
    keywordLine: string | null;
}

export interface ScheduleFLineOption {
    line: string;
    label: string;
}

interface ScheduleFMappingPanelProps {
    accounts: ScheduleFMappingAccount[];
    /** Persisted overrides: accountGuid → Schedule F line. */
...
export default function ScheduleFMappingPanel({
    accounts,
    mappings,
    lineOptions,
    saving,
    onSave,
}: ScheduleFMappingPanelProps) {
    ...
    /** The line an account actually lands on: override, else keyword, else 32. */
    const landingLine = (a: ScheduleFMappingAccount): string =>
        effectiveOverride(a.guid) || a.keywordLine || '32';
    ...
    return (
        <div className="space-y-3">
                                        ...
                                        {!override && (
                                            <p className="text-[11px] text-foreground-muted">
                                                ...
                                                {a.keywordLine
                                                    ...
                                                    : '32 · Other expenses (no keyword match)'}
                                            </p>
                                        )}
                                    </div>
                                </MobileCard>
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-background-tertiary">
                            <tr className="text-left text-xs text-foreground-muted">
                                ...
                                <th className="px-3 py-2 font-medium w-64">Schedule F line</th>
                            </tr>
                        </thead>
                        <tbody>
                            {visibleAccounts.map((a) => {
            ...
            <p className="text-[11px] text-foreground-muted">
                ...
                automatic mapping — unmatched accounts fall to line 32 &ldquo;Other expenses&rdquo;.
            </p>
        </div>
    );
}
