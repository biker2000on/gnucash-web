...
export function computeRealizedGain(
    splits: Array<{ shares: number; value: number; guid?: string }>,
    isClosed: boolean,
    carriedBasis = 0,
    ...
    if (isClosed) {
        ...
        return -traded.reduce((sum, s) => sum + s.value, 0)
            - carriedBasis
    ...
    const buyCost = buys.reduce((sum, s) => sum + Math.abs(s.value) + feeOf(s), 0) + carriedBasis;
...
export async function getLotsForAccounts(
    ...
    for (const lot of lots) {
        ...
        const totalCost = lotSplits
            .filter(s => s.shares > 0)
            .reduce((sum, s) => sum + Math.abs(s.value) + (fees.get(s.guid) ?? 0), 0) + carriedBasis;
