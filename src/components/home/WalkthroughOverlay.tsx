...
/**
 * Full-screen room-by-room capture stepper. One room at a time with a
 * rapid-entry form: "Add & next item" keeps you in the room, "Next room"
 * advances, "Finish" shows a recap of everything added this walk.
 */
export function WalkthroughOverlay({ rooms, onClose }: WalkthroughOverlayProps) {
    ...
    const addItem = async (): Promise<boolean> => {
        ...
        try {
            ...
            setAdded((prev) => [
                ...prev,
                { id: item.id, name: item.name, estValue: item.estValue, roomId: room.id },
            ]);
            resetForm();
            nameRef.current?.focus();
            return true;
    ...
    /** Add the pending form entry (if any) before moving on. */
    const flushPending = async (): Promise<boolean> => {
        if (name.trim()) return addItem();
        return true;
    };
