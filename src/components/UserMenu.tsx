...
export function UserMenu() {
    ...
    if (loading) {
        return (
            <div className="w-8 h-8 rounded-full bg-background-tertiary animate-pulse" />
        );
    }
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            >
                ...
                <svg className={`w-4 h-4 text-foreground-muted transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
            </button>

            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-background-secondary border border-border rounded-lg shadow-lg overflow-hidden z-50">
                    ...
                    <div className="py-1">
                        <Link
                            ...
                            className="w-full px-4 py-2 text-left text-sm text-foreground-secondary hover:bg-surface-hover transition-colors flex items-center gap-2"
                        >
