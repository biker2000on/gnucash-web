...
/** The payload a pop-out explain window receives: a token, a full trace, or both. */
...
/**
 * Fetches (by token, when no inline trace is supplied) and renders one
 * calculation trace. Shared by the Explain modal and /popout/explain.
 */
export function ProvenanceTraceView({ traceId, trace: suppliedTrace }: ExplainPayload) {
  ...
  const loading = !trace && !error && Boolean(traceId);

  useEffect(() => {
    if (suppliedTrace || !traceId) return;
...
export function ProvenanceModal({
  ...
  // When the pop-out window closes, the last shown trace re-docks here so the
  // explanation returns to the main window without losing state.
  ...
  const effectiveOpen = isOpen || redockPayload !== null;
  const payload: ExplainPayload = isOpen ? { traceId, trace } : (redockPayload ?? {});
  ...
  useLayoutEffect(() => {
    if (!isOpen || !isPopoutOpen) return;
    if (!traceId && !trace) return;
    if (showInPopout({ traceId: traceId ?? null, trace: trace ?? null })) onClose();
  }, [isOpen, isPopoutOpen, traceId, trace, showInPopout, onClose]);

  const handlePopout = () => {
    const id = payload.trace?.id ?? payload.traceId;
    if (!id && !payload.trace) return;
    const url = id ? `/popout/explain?trace=${encodeURIComponent(id)}` : '/popout/explain';
    const opened = openPopout(url, {
      traceId: payload.traceId ?? payload.trace?.id ?? null,
