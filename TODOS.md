...
- [x] **No real-Postgres concurrency harness.** Done 2026-08-19: `vitest.integration.config.ts` + `src/__tests__/integration/` against the CI postgres:17 service; real lock tests — `trading-account-lock-order`, `account-lock-hierarchy-deadlock`, `account-rename-reparent-race`, `simplefin-create-race`, `simplefin-transaction-id-dedupe`, `lot-outflow-ordering` (polly/ci-integration-harness). Stale "no harness" disclaimers in `reconciled-split.service.test.ts` and `bulk-edit.test.ts` removed 2026-08-19.
