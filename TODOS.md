...
- [x] **Rotate the committed app password** and move Playwright specs to
  ...
  inert. Verified closed 2026-08-19: `tests/e2e/review-mode.spec.ts` reads
  `E2E_USER`/`E2E_PASS` and skips without them, no credential literal remains
  anywhere under `tests/`, and the two dead root-level specs are gone.
...
- [x] Fake timers in "today"-relative tests (ASI-2-005). Still no `setSystemTime`/`useFakeTimers` (e.g. `lots-holding-period.test.ts`, `membership.service.test.ts`); only the TZ pin mitigates. — Done 2026-08-19: `vi.useFakeTimers({ toFake: ['Date'] })` + `vi.setSystemTime` pinned (and `vi.useRealTimers()` in `afterEach`) in the four clock-sensitive suites: `lots-holding-period.test.ts`, `membership.service.test.ts`, `invoice-engine.test.ts` (unpost reversal dated "today"), `InvestmentTransactionForm.test.tsx` (date-field shortcuts).
- [x] Webhook DNS re-resolution (ASI-3-007). `src/lib/webhooks.ts` still does not resolve before `fetch`. — Done 2026-08-19: delivery moved off fetch to node:http/https with a pinned dns.lookup that validates every resolved address against the module's private/loopback/link-local deny list, so the socket connects to the address we checked; covered by src/lib/__tests__/webhook-ssrf.test.ts. **Follow-up 2026-08-19:** the pin re-derived `allowInternal` from the literal hostname, which refused every LAN webhook with a *name* (`truenas`, `ntfy.lan`, `*.home.arpa`, Tailscale). It is now persisted as `allow_internal` on the row (idempotent ADD COLUMN in `ensureWebhooksTable`, pre-existing rows backfilled to TRUE), threaded through the create/update service + API + settings UI.
...
- [x] Dashboard SQL GROUP BY + invalidation debounce + idx TTLs (ASI-6-007). `dashboard/income-expense` and `sankey` still `findMany` + JS reduce; no debounce in `data-events-subscriber.ts`; `cache.ts` `zadd` without EXPIRE. — Done 2026-08-19: `sumSplitsByAccount(AndMonth)` numeric GROUP BY in both routes + pure `buildMonthlySeries`, 300 ms leading-edge per-book coalescing in `data-events-subscriber.ts`, and `EXPIRE … GT` on the zset indexes in `cache.ts`. **Follow-up 2026-08-19:** `GT` alone is a no-op on a key with no TTL (Redis treats non-volatile as infinite), so a fresh index never got one; `cache.ts` now pipelines `EXPIRE … NX` ahead of `EXPIRE … GT`.
...
- [x] Prisma ↔ db-init drift check in CI (ASI-7-006). No script/CI step; `sql/001-performance-indexes.sql` still present. — Done 2026-08-19: added scripts/check-schema-drift.ts (npm run schema:check) comparing prisma/schema.prisma models against every CREATE TABLE gnucash_web_* under src/lib/, with an explicit allowlist seeded from today's 52-table drift; wired into the deploy.yml quality job after docs:check, and deleted sql/001-performance-indexes.sql since db-init creates a strict superset.
...
- [x] ESLint ignore + triage unused-var warnings (ASI-8-002). **Partial 2026-08-19:** `eslint.config.mjs` now ignores `.worktrees/`, `.claude/`, `.polly/` (full lint 20 min → 43 s). Remaining: 7 warnings — unused `isMultiCurrency` (`api/transactions/route.ts`), `toCents` (`lib/reconcile.ts`), `EMP_A`, `_input`/`_init`, s-corp-analyzer `useMemo` deps. — Done 2026-08-19: 0 warnings and `npm run lint` is now `eslint --max-warnings 0`. `isMultiCurrency` was a real gap — the comment said "for audit log" but it never reached the payload; it is now written as `multi_currency`. `toCents` was a redundant import beside its own re-export; `EMP_A` became the assertion it was written for (no in-book guid list shipped either); `PAD` hoisted to module scope; `no-unused-vars` now honours the `^_` convention; one now-stale `eslint-disable` removed.
...
- [x] **Divergent balance tolerances across the codebase.** **Partial 2026-08-19:** settlements `0.011` removed and the create path is exact. Remaining: `0.01` at `mortgage.service.ts`, `capital-gains.ts`, `portfolio/route.ts`; `0.005` ×7 in `lot-scrub.ts` + `statement-reconcile.ts`; `0.0001` share epsilon across 21 non-test files (`DEFAULT_QTY_EPSILON` partially centralises). No single tolerance module. **Effort:** L. — Done 2026-08-19: new `src/lib/tolerances.ts` names every tolerance with a which-to-use-when doc; all listed sites migrated (lot-scrub balance invariants and reconcile1099B basis mismatch tightened from a whole cent to half a cent); tripwire `src/lib/__tests__/tolerance-literals.test.ts` fails on new bare literals with a ratcheting legacy budget.
...
- [x] **`lots.ts` reports gross while `capital-gains.ts` nets commissions.** **Partial 2026-08-19:** `getAccountLots({ includeTradeFees: true })` runs the same `loadTradeFees` allocator as the 8949 and nets fees into basis/proceeds; the Investment Lots report passes it and the ledger now books gains net of fees. Remaining: it is opt-in — `api/accounts/[guid]/lots/route.ts` and `api/reports/tax-harvesting/route.ts` still get gross figures. Make it the default or pass it there. **Effort:** S. — Done 2026-08-19: `getAccountLots` now nets by default (`includeTradeFees: false` is the gross escape hatch); both routes pass the book-scoped account-path map and surface fee warnings, and tax-harvesting was batched onto `getLotsForAccounts`. **Follow-up 2026-08-19:** `getLotsForAccounts` now defaults to netted too (the 8949 path passes an explicit `includeTradeFees: false` for its own allocation), and the sell planner + rebalancer moved off per-account `getAccountLots` onto one batched, path-classified call that surfaces `feeWarnings`.
...
- [x] **Short/oversold rows show a stale cost basis.** **Partial 2026-08-19:** pool removals clamp at held shares so an oversell drains basis to ~0 and coverage is nulled (no stale positive basis). Remaining: no short-position model; oversold rows show basis 0 / coverage unknown rather than a short basis. **Effort:** L. — Done 2026-08-19: `CostBasisPool` now carries a signed position (short leg + `shortProceeds`) in `src/lib/cost-basis.ts`; `positionSide`/`shortProceeds` are exposed by `getAccountHoldings`, the per-account transactions API (`position_side`), the portfolio and valuation routes, and rendered as "short basis (proceeds)" with its own mark in `HoldingsTable`/`InvestmentAccount`; short gain uses `proceeds − price × |qty|`. Lot assignment, scrub, realized gains and 8949 untouched.
- [x] **Ledger UI ignores the coverage field.** Verified open 2026-08-19: `AccountLedger.tsx` row type still declares only `cost_basis`; coverage marks landed on holdings/portfolio/dividends, not ledger rows. **Effort:** S. — Done 2026-08-19: `cost_basis_uncovered_shares` threaded through `AccountLedger` row type + `investment-utils` (`costBasisCoverageForRow`), and `InvestmentEditRow`'s three cost-basis cells render `CostBasisCoverageMark`.
- [x] **Email-ingest manual retry.** Verified open 2026-08-19 (descoped by design; `email-ingest.ts` header still documents the missing immutable owner column). **Effort:** L. — Done 2026-08-19: immutable owner snapshot (`owner_user_id`/`owner_book_guid`/`owner_sender_id`/`owner_sender_email`/`manual_retries`) added to `gnucash_web_ingest_messages` by `ensureEmailIngestTables` with a proof-only backfill; `claimIngestMessage` writes it once via COALESCE and the poller routes by the snapshot instead of re-matching the allowlist; `requestIngestRetry` + `POST /api/settings/email-ingest/messages/[id]/retry` re-arm a terminal failure for its snapshotted owner (or a book admin) only, and the settings panel gets a Retry action that is disabled with a reason for non-retryable rows.
...
- [x] **`reconciled-split.service.ts` guard lookups are not book-scoped.** Verified open 2026-08-19: zero `book` references in the module. **Effort:** M. — Done 2026-08-19: `ReconciledCheckOptions.bookAccountGuids` is now required and pushed into both `FOR UPDATE` lock statements and the post-lock split read, so an out-of-book guid is neither locked nor named; callers (transaction/packages/payslip-post services) thread their book.
- [x] **Inventory item accounts are nullable on save, required at post.** Verified open 2026-08-19: `inventory.service.ts` still writes nulls; `assertItemsPostable` rejects at fulfilment (now names all offenders, but still post-time). **Effort:** M. — Done 2026-08-19: new `post_to_ledger` opt-in on items (default on, legacy rows backfilled off); `createItem`/`updateItem` now run `validatePostingAccounts` (present + in-book + non-placeholder + right account type) and return a 400 `{ error, fields }`; item form marks the three accounts required and renders the per-field errors.
- [x] **Legacy average-cost fulfilments cannot post a COGS reversal.** Verified open 2026-08-19: `getShipmentWeightedUnitCost` returns null on any missing `unit_cost`; no backfill. **Effort:** M. — Done 2026-08-19: one-shot `db-init.ts` backfill derives legacy ship `unit_cost` from the shipment's posted COGS split, then the item cost, tagged in a new `unit_cost_source` column; where nothing is derivable `returnToStock` now posts the reversal at the item's current weighted cost with a `FulfillResult.warnings` entry and an 'estimated basis' ledger memo instead of hard-failing.
...
- [x] **Three copies of the depth-200 ancestor CTE.** Verified open 2026-08-19: `book-scope.ts`, `book-lock.ts`, `inventory-engine.ts`. **Effort:** S. — Done 2026-08-19: extracted to `src/lib/sql/ancestor-cte.ts` (`ancestorCte` + `MAX_ACCOUNT_DEPTH`), used by book-scope, book-lock and inventory-engine.
- [x] **Reconcile adjustment tests never assert a shared `tx_guid`.** Verified open 2026-08-19: `reconcile.test.ts` adjustment test still asserts shape + `assertBalanced` only; only `scu = 100`. **Effort:** S. — Done 2026-08-19: `reconcile.test.ts` pins both legs to the created transaction's guid and adds scu 1000 / scu 1 adjustment cases.
- [x] **Coverage epsilon is absolute, not magnitude-scaled.** Verified open 2026-08-19 (`commodities.ts` `qtyEpsilonForScu`, fails safe). **Effort:** S. — Done 2026-08-19: `qtyEpsilonWithMagnitude(scu, magnitude)` in `tolerances.ts` (REL 1e-9 over an absolute floor), applied in `commodities.ts` and the per-account transactions route, with long-replay regression tests at both.
- [x] **14 routes emit a generic `error: "Validation failed"` alongside specific `errors[]`.** **Partial 2026-08-19:** shared reader `src/lib/api-error.ts` added and the transactions create/update routes now set `error` to the joined summary (polly/h13-create-errors). Remaining: 13 routes still emit the generic string — auth/login, auth/register, auth/totp/{confirm,disable,recovery,verify}, commodities, exchange-rates, prices, prices/[guid], prices/audit, prices/fetch, user/profile. **Effort:** S. — Done 2026-08-19: new `src/lib/api-validation.ts` (`validationErrorResponse`) joins the Zod issues into a field-naming summary for `error`; all 13 remaining routes call it.
...
- [x] `CreateBookWizard.tsx` hand-rolls a *second* book-creation form hitting a **different endpoint** (`/api/books/from-template` vs `NewBookForm.tsx` → `/api/books/default`) with duplicated validation. Verified open 2026-08-19. — Done 2026-08-19: extracted `src/components/books/BookCreateForm.tsx` (name + shared `validateBookName` + submit state); NewBookForm and the wizard import step both compose it, keeping their own endpoints.
- [x] **Seven** hand-rolled "read the server error body" implementations. **Partial 2026-08-19:** `src/lib/api-error.ts` (`extractErrorMessage`/`readErrorBody`/`throwErrorBody`) exists and is wired into `AccountLedger.tsx`, `WeekGrid.tsx`, `ContinuousCloseDashboard.tsx`. Remaining: `InvestmentTransactionForm.tsx` (~:577) and `AccountLedger.tsx:1421` still hand-roll; ~99 other `.error || '…'` readers across `src/components`. — Done 2026-08-19: swept all 107 component-side readers (58 files) onto `extractErrorMessage` from `@/lib/api-error`, including InvestmentTransactionForm and AccountLedger:1421.
- [x] `InvestmentTransactionForm.tsx` — **write-only field** "Split Ratio (informational)", dead prop `accountCommodityGuid`, write-only state vars, dead `nameField` param. Verified open 2026-08-19 (only a11y commits touched the file). — Done 2026-08-19: removed the `accountCommodityGuid` prop (and its 2 call sites), the write-only `splitRatio` field (no ratio exists in the /api/transactions payload) and the `*AccountName` state / `nameField` param.
...
- [x] `AccountForm.tsx`, `BudgetForm.tsx`, `LoginForm.tsx` — `rounded-xl`/`2xl` off `DESIGN.md`'s radius scale; three different input recipes across sibling forms. Verified open 2026-08-19. — Done 2026-08-19: all 446 `rounded-xl`/`2xl` across src normalised to `rounded-lg`; the three forms now use the shared `INPUT`/`SELECT`/`TEXTAREA`/`inputClass()` recipe in `src/components/ui/form.tsx`; DESIGN.md closes the radius scale explicitly.
- [x] Native `title=` tooltips, **banned by `DESIGN.md`** — `InvestmentTransactionForm.tsx:645` ("Return of Capital" explanation only in a native tooltip), `:712`, and ~72 component files. Verified open 2026-08-19. — Done 2026-08-19: all 352 native `title=` sites on 133 files converted to the new `Tip` primitive (`src/components/ui/Tooltip.tsx`, clones its child so layout/table validity are untouched) or to `aria-label` for form controls and icon-only buttons; tripwire `src/__tests__/no-native-title.test.ts` parses every `.tsx` and fails on any new one.
- [x] `AccountLedger.tsx` — `☑`/`☐` text glyphs as checkbox chrome (no `aria-pressed`/`role="checkbox"`). Verified open 2026-08-19. — Done 2026-08-19: replaced with `src/components/ui/CheckboxChip.tsx` (`<button role="checkbox" aria-checked>` + inline SVG check); tests in `src/components/ui/__tests__/checkbox-chip.test.tsx`.
- [x] `--error` and `--negative` are the same colour (`#dc2626` / dark `#f87171`) under two token names (`globals.css`). Verified open 2026-08-19. — Done 2026-08-19: kept both (DESIGN.md names them as distinct roles), documented the split in `globals.css` + DESIGN.md — `--negative` is money below zero, `--error` is system state — and migrated 88 non-money uses of `--negative` (validation, failed requests, destructive controls, required markers) onto `--error`.
- [x] Server errors via toast, not inline, not announced. **Partial 2026-08-19:** `src/components/a11y/LiveRegion.tsx` (always-mounted `aria-live`) is wired into 25 components; `ToastContainer` has `aria-live="polite"`; `SplitRow`/`ReconciliationPanel` use `role="alert"` (polly/ui-a11y-remainder). Remaining: `TransactionForm.tsx` (~:1037) renders server/validation errors in a plain list with no live region; `AccountLedger` save failures still go to toast rather than inline on the field. — Done 2026-08-19: `TransactionForm.tsx` now mounts an `ErrorLiveRegion` and renders each message inline under the offending control (date/description/amount/both accounts/splits); `api-error.ts` gained `extractFieldErrors()` + `ApiRequestError` so server `errors[]` survive the throw from `TransactionFormModal`; `AccountLedger` inline-save failures now also pin the reason to the failed row (`ledger/RowSaveErrorRow.tsx`) and announce once. Tests: `TransactionForm.server-errors.test.tsx`, `row-save-error-row.test.tsx`, `api-error.test.ts`.
...
- [x] **CI has no `pull_request` trigger.** `.github/workflows/deploy.yml` fires on `push: branches: [main]` only. Verified open 2026-08-19. Note: main is currently gated by the local-CI workflow (`[skip ci]` pushes after a full WSL reproduction of the quality job) to conserve Actions minutes. — Done 2026-08-19: `pull_request: branches: [main]` (paths-ignore `**.md`/`docs/**`) added to `deploy.yml`; `build-and-push` is now `if: github.event_name == 'push'`, plus a PR-only cancel-in-progress concurrency group.
- [x] **`npm run build` is not in the `quality` job.** Verified open 2026-08-19: quality = typecheck, docs:check, test:run, integration schema+tests, deadlock oracle, lint; build only in the Docker step. — Done 2026-08-19: `deploy.yml` runs `npm run build` after typecheck with placeholder `DATABASE_URL`/`SESSION_SECRET` and an `actions/cache` entry for `.next/cache` keyed on lockfile + sources.
- [x] **The Playwright E2E suite runs nowhere.** Verified open 2026-08-19: no `test:e2e` script, no CI step. — Done 2026-08-19: `test:e2e`/`test:e2e:ui` scripts, a `workflow_dispatch` + weekly `.github/workflows/e2e.yml` (postgres service, `test:integration:schema`, build, Playwright chromium, report artifact), and a README section on running it locally. Deliberately off the push/PR path to conserve Actions minutes.
...
- [x] **`test:coverage` exists but CI never invokes it**, so there is no coverage threshold. Verified open 2026-08-19. — Done 2026-08-19: the missing `@vitest/coverage-v8` dependency added (the script could not run at all), `deploy.yml` runs `test:coverage` in place of `test:run` (same single execution), `vitest.config.ts` gained `reportOnFailure` + regression-floor thresholds (58/48/55/60 against measured 61.66/51.73/58.41/63.29), and the lcov/json summary is uploaded as an artifact.
