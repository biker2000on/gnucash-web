# Investment Cost Basis Carry-Over — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Track historical cost basis across investment account transfers. When shares are transferred between brokerage accounts, carry over the original purchase cost basis instead of showing $0.

**Architecture:** Create a server-side cost basis tracing utility that follows transfer chains through the GnuCash splits data. Support lot-based tracing when available, with FIFO/LIFO/average fallback. Add user preferences for toggle and method. Integrate into the investment transaction API and holdings calculations.
...
|--------|------|----------------|
| Create | `src/lib/cost-basis.ts` | Core cost basis tracing logic |
| Modify | `src/contexts/UserPreferencesContext.tsx` | Add costBasisCarryOver + costBasisMethod prefs |
| Modify | `src/app/api/accounts/[guid]/transactions/route.ts` | Use traced cost basis for investment accounts |
| Modify | `src/lib/commodities.ts` | Integrate tracing into calculateCostBasis |
| Modify | Holdings-related components and APIs | Use traced cost basis in portfolio views |

---

### Task 1: Create Cost Basis Tracing Utility

**Files:**
- Create: `src/lib/cost-basis.ts`

- [ ] **Step 1: Create the core tracing module**

```typescript
// src/lib/cost-basis.ts
import prisma from './prisma';
import { toDecimal } from './db';
...
export function isTransferIn(
  ...
  allSplits: Array<{ quantity_num: bigint; quantity_denom: bigint; account_guid: string; account_name?: string }>,
...
export async function traceCostBasis(
  ...
  // Note: if post_date is null, falls back to current date — acceptable trade-off
  ...
  result.tracedFromAccount = sourceSplit.account?.name;
  cache.set(cacheKey, result);
  return result;
}
...
function isTransferInSplit(
  split: { quantity_num: bigint; quantity_denom: bigint; transaction?: { splits: Array<{ account_guid: string; quantity_num: bigint; quantity_denom: bigint; account?: { commodity_guid?: string | null } | null }> } | null },
...
```bash
git add src/lib/cost-basis.ts
git commit -m "feat: add cost basis tracing utility with FIFO/LIFO/average support"
```

---

### Task 2: Add Cost Basis User Preferences
...
Follow the exact pattern of existing preferences. Load from stored prefs, save with optimistic update + localStorage + API POST.

- [ ] **Step 3: Add toggle and method selector to settings UI**

Find the existing preferences/settings component and add:
- Toggle switch for "Carry over cost basis on transfers"
- Segmented control or radio for FIFO/LIFO/Average (only visible when toggle is on)

- [ ] **Step 4: Commit**

```bash
git add src/contexts/UserPreferencesContext.tsx
git commit -m "feat: add cost basis carry-over user preferences"
```

---

### Task 3: Integrate into Investment Transaction API

**Files:**
- Modify: `src/app/api/accounts/[guid]/transactions/route.ts:161-205`

- [ ] **Step 1: Import and use cost basis tracing**
...
- [ ] **Step 2: Read user preferences from request or query params**

Add query params for cost basis preferences:
```typescript
...
- [ ] **Step 3: Modify the investment running totals calculation**

In the investment running totals loop (~lines 161-205), after computing share balance and cost basis, check if the split is a transfer-in and if carry-over is enabled:

```typescript
// Create request-scoped cache
const costBasisCache = createCostBasisCache();

// In the loop where running totals are computed:
for (const tx of orderedSplits) {
  const shares = toDecimal(tx.quantity_num, tx.quantity_denom);
  const value = Math.abs(toDecimal(tx.value_num, tx.value_denom));

  if (shares > 0) {
    runShares += shares;

    // Check if this is a transfer-in
    if (costBasisCarryOver && isTransferIn(tx, tx.transaction.splits, accountCommodityGuid)) {
      const traced = await traceCostBasis(tx.guid, costBasisMethod, accountCommodityGuid, shares, costBasisCache);
    ...
    const avgCost = runShares > 0 ? runCostBasis / runShares : 0;
    runCostBasis -= avgCost * soldShares;
    runShares += shares;
  }

  totalsMap.set(tx.tx_guid, { shareBalance: runShares, costBasis: runCostBasis });
}
```
...
```bash
git add src/app/api/accounts/[guid]/transactions/route.ts
git commit -m "feat: integrate cost basis tracing into investment transaction API"
```

---

### Task 4: Integrate into Holdings/Portfolio

**Files:**
- Modify: `src/lib/commodities.ts`
- Modify: `src/app/api/investments/portfolio/route.ts` — use traced cost basis in portfolio calculations
- Modify: Holdings display component (find via `grep -r 'HoldingsTable\|getAccountHoldings' src/`)

- [ ] **Step 1: Update calculateCostBasis in commodities.ts**

Modify `calculateCostBasis` to optionally accept traced cost basis:
...
- [ ] **Step 2: Update holdings/portfolio API to use tracing**

In the portfolio API endpoint, when computing cost basis per holding:
- Check user preferences for carry-over setting
- If enabled, use `traceCostBasis` for transfer-in splits
- Sum the traced cost basis into the holding total

- [ ] **Step 3: Pass cost basis preferences from client to API**

In the investment ledger and holdings components, include the cost basis preferences as query params when fetching:
```typescript
...
Run: `npm run dev`
Test:
- Investment ledger shows traced cost basis for transfer-in rows
- Holdings page reflects traced cost basis
- Toggle preference off → shows $0 cost basis for transfers
- Switch between FIFO/LIFO/Average → cost basis values change accordingly

- [ ] **Step 5: Commit**

```bash
git add src/lib/commodities.ts
git commit -m "feat: integrate cost basis carry-over into holdings and portfolio views"
```
