...
8. `prices(commodity_guid, currency_guid, date)`; `commodities(namespace, mnemonic)`; `accounts(parent_guid, name)`; partial unique on `gnucash_web_transaction_meta(simplefin_transaction_id)`; partial unique on `gnucash_web_reconciliation_sessions(account_guid) WHERE status='started'`; unique or dedup table for funding-sweep `num`; unique on `slots(obj_guid, name)` for counters. (Some need a dedupe pass on existing data first.)
   ...
   > **Correction (2026-08-17), two of these keys do not exist and must not be added** — see `src/lib/db-init.ts` for the full reasoning. `slots(obj_guid, name)`: GnuCash KVP lists legitimately repeat a name per object. `accounts(parent_guid, name)`: the `accounts` table also stores scheduled-transaction templates, which have one child account PER SPLIT all named `''` under the same parent, so a unique index there makes every multi-split scheduled transaction fail with 23505 — in this app and in GnuCash desktop against the same book. A partial index cannot exclude them (predicates see only the row's own columns; the marker is an ancestor). H4/H7 are served by the per-`(parent, name)` advisory lock instead, and db-init drops the index if an earlier release created it.
