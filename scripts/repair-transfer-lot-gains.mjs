...
// ── Tax classification (mirrors classifyAccountTax in src/lib/lot-scrub.ts) ──
// The engine deliberately books NO gains transaction for a lot in a
// TAX_EXEMPT account (Roth / HSA): a sheltered gain is not income, so a
// closed sheltered lot with no gains offset is the engine's CORRECT end
// state, not a defect. Part (b) therefore reports those separately instead
// of counting them as "unbooked".
const allAccounts = (await c.query(
  `SELECT guid, name, parent_guid, account_type FROM accounts`,
)).rows;
const acctByGuid = new Map(allAccounts.map(a => [a.guid, a]));
const bookRootGuids = new Set(
  (await c.query(`SELECT root_account_guid FROM books`)).rows
    .map(r => r.root_account_guid).filter(Boolean),
);
let prefByAcct = new Map();
try {
  prefByAcct = new Map(
    (await c.query(
      `SELECT account_guid, is_retirement, retirement_account_type
       FROM gnucash_web_account_preferences`,
    )).rows.map(r => [r.account_guid, r]),
  );
} catch { /* preferences table absent on a bare GnuCash DB */ }

function classifyRetirementType(type) {
...
function classifyAccountTax(accountGuid) {
  const names = [];
  const seen = new Set();
  let cur = accountGuid;
  while (cur && !seen.has(cur)) {
    seen.add(cur);
    const acct = acctByGuid.get(cur);
    if (!acct) break;
    // Nearest explicit preference (self first) wins, as in the engine.
    const pref = prefByAcct.get(cur);
    if (pref?.is_retirement && pref.retirement_account_type) {
      return classifyRetirementType(pref.retirement_account_type);
    }
    if (pref && pref.is_retirement === false && pref.retirement_account_type === 'brokerage') {
      return 'TAX_NORMAL';
    }
    names.push(String(acct.name ?? '').toLowerCase());
    if (acct.account_type === 'ROOT' || bookRootGuids.has(cur) || !acct.parent_guid) break;
    cur = acct.parent_guid;
  }
  const joined = names.join(' ');
  if (/\broth\b/.test(joined) || /\bhsa\b/.test(joined)) return 'TAX_EXEMPT';
  if (/\bira\b/.test(joined) || /\b401k?\b/.test(joined) || /\b403b?\b/.test(joined) || /\b457\b/.test(joined)) return 'TAX_DEFERRED';
  return 'TAX_NORMAL';
}
...
for (const lot of lots) {
  ...
  // A TAX_EXEMPT (Roth/HSA) lot is SUPPOSED to close without a gains split —
  // generateCapitalGains skips those by design. Tally separately.
  if (classifyAccountTax(lot.account_guid) === 'TAX_EXEMPT') {
    exemptCount++;
    exemptTotal += -a.valueSum;
    continue;
  }
  reportCount++;
...
console.log(`  (${exemptCount} closed lots in TAX_EXEMPT (Roth/HSA) accounts carry ${exemptTotal.toFixed(2)} of sheltered gain with no gains split — the engine's intended state, nothing to book.)`);
